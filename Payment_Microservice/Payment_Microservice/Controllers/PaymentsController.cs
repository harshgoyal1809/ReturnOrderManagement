...
namespace Payment_Microservice.Controllers
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        public IActionResult GetpaymentDetails(int requestId, long cardNumber, double creditLimit, double processingCharge)
        {
            try
            {
                log.Info("in Payment Service");
                log.Info("Start GetBalance");
                return Ok(paymentRepositoryImp.processPaymentService(requestId, cardNumber, creditLimit, processingCharge));
            }
            catch (Exception e)
            {
